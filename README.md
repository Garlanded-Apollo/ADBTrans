...
- **列表排序** — 点击表头按名称、大小、修改时间或类型排序，再次点击切换正序/倒序，文件夹始终排在文件前面
- **列宽调整** — 拖拽调整各列宽度
- **文件搜索** — 支持当前目录即时筛选，以及从根目录 `/` 开始的全盘文件名搜索（自动跳过 /proc、/sys、/dev）
...
- **自动提醒** — 启动后及每 6 小时自动检查更新，发现新版本时侧边栏左下角图标显示红点，点击直达更新页面
...
│               ├── file/            # 文件浏览
│               │   ├── FileTable.tsx       # 文件列表表格（排序、多选、搜索、列宽调整）
